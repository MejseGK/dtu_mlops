...
def corrupt_mnist() -> tuple[torch.utils.data.Dataset, torch.utils.data.Dataset]:
    ...
    train_images = train_images.unsqueeze(1).float() # Add channel dimension
    test_images = test_images.unsqueeze(1).float() # Add channel dimension
    train_target = train_target.long() # Convert from int to long/float for classification
    test_target = test_target.long() # Convert from int to long/float for classification
...
if __name__ == "__main__":
    ...
    print(f"Shape of a training point {(train_set[0][0].shape, train_set[0][1].shape)}") # should be for image: (1, 28, 28); and target: scalar (torch.Size[])
    print(f"Shape of a test point {(test_set[0][0].shape, test_set[0][1].shape)}") # should be for image: (1, 28, 28); and target: scalar (torch.Size[])
